...
export async function fetchCurrentPrice(ticker) {
  const yahooPrice = await fetchPriceFromYahooChart(ticker);
  if (yahooPrice > 0) return yahooPrice;
  return fetchCurrentPriceFromStooq(ticker);
}
...
export async function ensureCurrentPrice(data, ticker) {
  ...
  const price = await fetchCurrentPrice(ticker).catch(() => 0);
  if (!(price > 0)) {
    console.warn(`[Price Fallback] No live price found for ${ticker}`);
    return data;
  }

  console.log(`[Price Fallback] Patched ${ticker} current_price=$${price.toFixed(2)} (source=${data.source || 'unknown'})`);
