...
def _build_response(status, body_dict):
    return {
        'statusCode': status,
        'headers': {
        ...
        'body': json.dumps(body_dict, allow_nan=False),
    }
...
def handler(request):
    try:
        ...
        if not ticker:
            return _build_response(400, {'error': 'Missing ticker parameter'})

        if yf is None:
            return _build_response(500, {'error': 'yfinance not available in this runtime'})

        t = yf.Ticker(ticker)

        # Market/price info (best-effort)
        info = {}
        try:
            info = t.info or {}
        except Exception:
        ...
        try:
            ...
            if is_df is not None and not is_df.empty:
                ...
                # Latest FY for top-level fields
                latest_col = cols[0]
                ...
                # Latest FCF (from cash flow if possible)
                fcf_latest = None
            ...
            # Fallback only with minimal info
            pass

        # Shares outstanding and names
